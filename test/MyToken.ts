...
describe("MyToken", () => {
  ...
  describe("TransferFrom", () => {
    ...
    it("should be reverted with insufficient allowance error", async () => {
      const signer0 = signers[0];
      const signer1 = signers[1];
      await expect(myTokenC.connect(signer1).transferFrom(signers[0].address, signer1.address, hre.ethers.parseUnits("1", decimals))).to.be.revertedWith("insufficient allowance");
    });
  });

  // Assignment 2
  describe("Assignment 2", () => {
    it("signer1 should have 10MT", async () => {
      const signer0 = signers[0];
      const signer1 = signers[1];
      await myTokenC.connect(signer0).approve(signer1.address, hre.ethers.parseUnits("10", decimals));
      await myTokenC.connect(signer1).transferFrom(signer0.address, signer1.address, hre.ethers.parseUnits("10", decimals));
